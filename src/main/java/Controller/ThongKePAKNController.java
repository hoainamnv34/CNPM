...
public class ThongKePAKNController implements Initializable {
    ...
    private void printePiechart(int quySelected) {
        ...
        try {
            ...
            String query;
            if(quySelected != 0) {
                query= "SELECT COUNT(*) FROM dbo.PhanAnhKienNghi WHERE  PhanHoi IS NOT NULL AND (MONTH(NgayPA) BETWEEN (" + quySelected + "-1)*3 + 1 AND "
                 + quySelected + "*3);";
            }else {
            ...
            if(quySelected != 0)
                query = "SELECT COUNT(*) FROM dbo.PhanAnhKienNghi WHERE MONTH(NgayPA) BETWEEN (" + quySelected + "-1)*3 + 1 AND "
                + quySelected + "*3;";
            else
